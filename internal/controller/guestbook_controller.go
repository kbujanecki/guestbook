...
var (
	setupLog = ctrl.Log.WithName("controllers").WithName("Guestbook")
)
...
func (r *GuestbookReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	logger.Info("reconciling guestbook: ", "req", req)
...
func (r *GuestbookReconciler) SetupWithManager(mgr ctrl.Manager) error {
	uber := NewWatchedResourcePredicateProvider()
	return ctrl.NewControllerManagedBy(mgr).
		For(&webappv1.Guestbook{}, builder.WithPredicates(uber.ForWrite())).
		Watches(
			&appsv1.StatefulSet{},
			&handler.EnqueueRequestForObject{},
			builder.WithPredicates(uber.ForRead()),
		).Complete(r)
}
